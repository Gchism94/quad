...
> Cairn was developed under the working name Quad. Commits and issues before
> 2026-08 use the old name.
...
**Environment variables renamed.** As of the Cairn rename, every `QUAD_*`
variable is `CAIRN_*` (for example `QUAD_DATABASE_URL` is now
`CAIRN_DATABASE_URL`). There is no fallback to the old names. Update any
local `.env` before starting the server.
...
products. Cairn targets the gap none of them fill:
...
- **Portable autograding.** A host-neutral grading spec runs in Cairn's own
...
No database daemon, no build tags, no configuration required. Cairn creates
`cairn.db` in the working directory on first run and keeps it across restarts.
...
go run ./cmd/cairn    # starts the control plane on :8080
```
...
CAIRN_WEB_DIR=web/dist go run ./cmd/cairn    # API + dashboard on :8080
```

Specific API routes always take precedence; everything else is served from
`CAIRN_WEB_DIR` with SPA fallback.
...
|---|---|
| `CAIRN_STORE=memory` | In-memory (ephemeral — lost on restart; useful for tests) |
| `CAIRN_STORE=sqlite` | SQLite at `CAIRN_SQLITE_PATH` (default: `cairn.db`) |
| `CAIRN_STORE=postgres` | PostgreSQL via `CAIRN_DATABASE_URL` |
| `CAIRN_DATABASE_URL` set | PostgreSQL (auto-detected) |
| _(nothing set)_ | **SQLite `cairn.db`** — zero-config default |

### SQLite (default)

```sh
go run ./cmd/cairn                       # creates cairn.db in the working dir
CAIRN_SQLITE_PATH=/var/lib/cairn/cairn.db go run ./cmd/cairn   # explicit path
```
...
CAIRN_DB_AUTOMIGRATE=1 go run ./cmd/cairn
```

`CAIRN_DB_AUTOMIGRATE=1` applies the embedded schema on startup (idempotent).
...
```sh
export CAIRN_ADMIN_USERS=alice,bob                       # allowlisted operator usernames
export CAIRN_GITHUB_CLIENT_ID=...                        # OAuth app (shared with student flow)
export CAIRN_GITHUB_CLIENT_SECRET=...
export CAIRN_OAUTH_REDIRECT_URL=https://your-host/auth/callback
export CAIRN_COOKIE_SECURE=1                             # set behind HTTPS
CAIRN_WEB_DIR=web/dist go run ./cmd/cairn
```

When `CAIRN_ADMIN_USERS` is unset the server runs **open** (no auth) and logs a
warning — fine for local development, not for anything exposed. `CAIRN_AUTH_DISABLED=1`
...
single-binary build (`CAIRN_WEB_DIR=web/dist`), not the split Vite dev server; for
...
The GitHub App backing the `CAIRN_GITHUB_APP_ID` installation needs these
...
Cairn is host-agnostic: the same interface (`pkg/adapter.Adapter`) that backs the
...
```sh
export CAIRN_FORGEJO_BASE_URL=https://forgejo.example.org  # instance root
export CAIRN_FORGEJO_TOKEN=...                              # personal/admin token
go run ./cmd/cairn
```

Both `CAIRN_FORGEJO_BASE_URL` and `CAIRN_FORGEJO_TOKEN` must be set together, or
...
```sh
export CAIRN_FORGEJO_OAUTH_CLIENT_ID=...      # OAuth2 app client id
export CAIRN_FORGEJO_OAUTH_CLIENT_SECRET=...  # OAuth2 app client secret
# CAIRN_FORGEJO_BASE_URL and CAIRN_OAUTH_REDIRECT_URL are reused from above
```

The redirect URI registered with the OAuth2 app must be the same
`CAIRN_OAUTH_REDIRECT_URL` used for GitHub (e.g. `https://your-host/auth/callback`);
...
choose another (or to be explicit), set `CAIRN_OPERATOR_HOST` to `github`,
`forgejo`, `gitea`, or `gitlab`:

```sh
export CAIRN_OPERATOR_HOST=forgejo   # github | forgejo | gitea | gitlab
```
...
signals Cairn's orchestrator to run grading on its own sandboxed container runners
...
Private clones authenticate via `GIT_ASKPASS` — the token from `CAIRN_FORGEJO_TOKEN`
...
username (`oauth2` by default; override with `CAIRN_FORGEJO_GIT_USERNAME` if your
...
> `CAIRN_FORGEJO_GIT_USERNAME=<token-owner-username>` to match the instance's
> expectation.
...
```sh
export CAIRN_GRADER=container            # sandboxed container runner (recommended)
export CAIRN_GRADER_IMAGE=python:3.12    # default image (a spec may set its own)
export CAIRN_GRADER_RUNTIME=docker       # or podman
export CAIRN_GIT_CLONE_TOKEN=...         # GitHub token for private repo clones
CAIRN_FORGEJO_TOKEN=...  go run ./cmd/cairn   # Forgejo token reused for grading
```
...
follows each host's base URL** (`CAIRN_FORGEJO_BASE_URL` / `CAIRN_GITLAB_BASE_URL` /
`CAIRN_GITHUB_BASE_URL`) and defaults to `https`, so a plain-HTTP instance (e.g.
`http://localhost:3000`) is cloned over `http` rather than forced through TLS. Two
optional env vars affect clone behaviour:

| Variable | Default | Purpose |
|---|---|---|
| `CAIRN_GITHUB_BASE_URL` | _(github.com)_ | GHES: set to your enterprise URL so the clone target matches your API base. |
| `CAIRN_FORGEJO_GIT_USERNAME` | `oauth2` | URL username for Forgejo HTTPS auth. Change to the token owner's username if your instance requires it. |
...
`CAIRN_GRADER_RESTRICTED_NETWORK`; with none configured it falls back to `none`, so
...
`CAIRN_GRADER_USER` to override — for example to a fixed non-root uid. If the server
...
`CAIRN_GRADER=local-exec-unsafe` selects the host exec runner, which has **no
isolation** (only a timeout) and is for trusted/local material only.

## Webhooks (auto-regrade on push)

When `CAIRN_WEBHOOK_BASE_URL` is set, Cairn registers a push webhook on each
...
```sh
# Public BASE URL of this Cairn instance. Cairn appends /webhooks/<host> per repo.
# It must be reachable BY THE GIT HOST, not just by you.
export CAIRN_WEBHOOK_BASE_URL=https://your-host   # (CAIRN_WEBHOOK_URL is a deprecated alias)

# Per-host signing secret; set the same value in the host's webhook config.
export CAIRN_FORGEJO_WEBHOOK_SECRET=$(openssl rand -hex 32)   # covers forgejo AND gitea
...
everywhere in Cairn, no student names, SIS IDs, or plaintext emails are stored or
...
- **Interoperability primitives** (`pkg/adapter`, `pkg/gradingspec`) —
  **Apache-2.0**. The pieces that make Cairn interoperable are maximally reusable.
